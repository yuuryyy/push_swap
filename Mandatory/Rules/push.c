...
/*   Updated: 2024/03/05 17:50:16 by ychagri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	a(t_list *node_b, t_list **stack_b, t_list *node_a, t_list **stack_a)
{
	int	pos;
	int	pos_t;
...
void	p_a(t_list *node_b, t_list **stack_b, t_list *node_a, t_list **stack_a)
{
	...
	else
	{
		...
		pos = median_check(node_b, *stack_b);
		if (pos == pos_t
			|| position(node_a, *stack_a) == position(node_b, *stack_b))
		{
			if (pos == 0
				|| position(node_a, *stack_a) == position(node_b, *stack_b))
				rr_a(stack_a, stack_b, node_a, node_b);
			else
				rrrn(stack_a, stack_b, node_a, node_b);
		}
		else
			a(node_b, stack_b, node_a, stack_a);
		if (node_a == *stack_a && node_b == *stack_b)
			push(stack_b, stack_a, PA);
	}
}

void	b(t_list *node_a, t_list **stack_a, t_list *node_b, t_list **stack_b)
{
	int	pos;
	int	pos_t;
...
void	p_b(t_list *node_a, t_list **stack_a, t_list *node_b, t_list **stack_b)
{
		...
		|| position(node_a, *stack_a) == position(node_b, *stack_b))
	{
		...
		b(node_a, stack_a, node_b, stack_b);
	if (node_a == *stack_a && node_b == *stack_b)
		push(stack_a, stack_b, PB);
}
